...
% x2_start = 0;
% for i=pos
%     [x1_start,x2_start] = td3(x1_start,x2_start,input(i),r,ts,h0);
%     x1(i) = x1_start;
%     x2(i) = x2_start;
% end
% figure(1);
...
% z3_start = 0;
% for i=pos
%     [z1_start,z2_start,z3_start] = eso3(z1_start,z2_start,z3_start, ...
...
for i=pos
    if i > 2
        ...
        %过系统函数
        output(i) = -den(2)*output(i-1)-den(3)*output(i-2)+num(2)*u(i-1)+num(3)*u(i-2);
        output(i) = output(i)+rand(1,1)/1000;
        %eso 模块
        [z1_start,z2_start,z3_start] = eso3(z1_start,z2_start,z3_start, ...
                                    beta_01,beta_02,beta_03, ...
